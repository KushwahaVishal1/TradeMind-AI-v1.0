...
def build_cost_feasibility_panel(
    ...
    feasible = (
        observed_ic is not None and math.isfinite(observed_ic) and observed_ic >= breakeven_ic
    )
    ...
    if observed_ic is None or not math.isfinite(observed_ic):
        ...
        verdict, headline = (
            Verdict.GOOD,
            (f"Measured IC {observed_ic:.3f} clears the {breakeven_ic:.3f} breakeven."),
        )
    else:
        verdict, headline = (
            Verdict.BAD,
    ...
    return Panel(
        ...
        metrics=(
            ...
            Metric(
                ...
                observed_ic,
                decimals=3,
                baseline=breakeven_ic,
                baseline_label="breakeven",
            ),
...
def build_skill_panel(
    ...
    auc_metric = Metric(
        "ROC-AUC", auc, stderr=auc_stderr, baseline=0.5, baseline_label="chance"
    )
...
def build_accounting_panel(max_reconciliation_error: float | None) -> Panel:
    ...
    return Panel(
        ...
        (
            "Balance-sheet and flow equity agree to within a paisa across every session."
            if ok
            else f"Equity paths disagree by up to {max_reconciliation_error:,.4f}. "
            "The backtest numbers cannot be trusted."
        ),
        metrics=(Metric("Max reconciliation error", max_reconciliation_error, decimals=8),),
    )
...
def build_performance_panel(
    ...
    if bench_return is not None and total is not None and total < bench_return:
        ...
        verdict = Verdict.CONCERN
        headline = (
            f"{total:+.2%} net, with {cost_drag:.2%} lost to costs — costs exceed the return."
        )
    else:
...
def build_overview(state: dict) -> list[Panel]:
    ...
    return [
        build_cost_feasibility_panel(
            state.get("round_trip_cost"),
            state.get("breakeven_ic"),
            state.get("observed_ic"),
            state.get("holding_days", 1),
        ),
        build_skill_panel(
            state.get("auc"),
        ...
        build_data_quality_panel(
            state.get("n_dq_errors", 0),
            state.get("n_dq_warnings", 0),
        ),
...
def build_drift_panel(summary: dict[str, float] | None) -> Panel:
    ...
    headline = (
        ...
        if n_sig
        else "No feature shows significant distribution shift."
    )
    ...
    return Panel(
        ...
        metrics=(
            ...
            Metric("Moderate (PSI > 0.10)", summary.get("n_moderate"), decimals=0),
...
def build_detectability_panel(windows: list) -> Panel:
    ...
    if not windows:
        return Panel("Monitoring sensitivity", Verdict.UNKNOWN, "No resolved predictions yet.")
    ...
    if not reportable:
        return Panel(
            ...
            metrics=tuple(
                Metric(f"{w.window_days}d observations", float(w.n_observations), decimals=0)
                for w in windows
            ),
        )
    ...
    return Panel(
        ...
        metrics=tuple(
            Metric(
                f"{w.window_days}d detectable change",
        ...
        caveats=(
            ...
            "An unchanged performance metric is therefore not evidence of health.",
        ),
    )
