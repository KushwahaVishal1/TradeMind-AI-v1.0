...
def inp(**kw) -> DecisionInput:
    base = dict(
        symbol="RELIANCE.NS", decision_date=TODAY,
        calibrated_probability=0.60, expected_return=0.01,
        volatility=0.25, position=None,
    )
    base.update(kw)
...
def test_confident_but_unprofitable_is_rejected():
    ...
    d = generate_signal(
        inp(calibrated_probability=0.72, expected_return=0.0003), TH
    )
...
def test_position_cap_never_force_closes():
    ...
    engine = DecisionEngine(TH, RiskEngine(max_positions=5))
    d = engine.decide(inp(n_open_positions=99, position=LONG,
...
def test_risk_gates_run_before_the_signal():
    ...
    d = engine.decide(inp(data_age_sessions=99, calibrated_probability=0.99,
                          expected_return=0.5))
...
def buy_decision(p=0.70):
    return Decision(symbol="X", decision_date=TODAY, signal=Signal.BUY,
                    calibrated_probability=p, expected_return=0.01)
...
def test_volatility_targeting_shrinks_volatile_names():
    calm = size_position(buy_decision(), inp(volatility=0.15),
                         "volatility_target", max_weight=0.10)
    wild = size_position(buy_decision(), inp(volatility=0.80),
                         "volatility_target", max_weight=0.10)
    assert wild < calm
...
def test_volatility_targeting_respects_the_cap():
    ...
    w = size_position(buy_decision(), inp(volatility=0.01),
                      "volatility_target", max_weight=0.10)
...
def test_volatility_targeting_equalises_risk_contribution():
    ...
    a = size_position(buy_decision(), inp(volatility=0.40),
                      "volatility_target", max_weight=0.10,
                      reference_volatility=0.25)
    b = size_position(buy_decision(), inp(volatility=0.80),
                      "volatility_target", max_weight=0.10,
                      reference_volatility=0.25)
...
def test_missing_volatility_falls_back_rather_than_guessing():
    w = size_position(buy_decision(), inp(volatility=None),
                      "volatility_target", max_weight=0.10)
    assert 0 < w < 0.10


def test_confidence_sizing_scales_with_probability():
    low = size_position(buy_decision(0.56), inp(), "confidence",
                        max_weight=0.10, buy_threshold=0.55)
    high = size_position(buy_decision(0.95), inp(), "confidence",
                         max_weight=0.10, buy_threshold=0.55)
    assert high > low
...
def test_batch_prioritises_the_strongest_signals():
    ...
    inputs = [
        DecisionInput(f"S{i}", TODAY, calibrated_probability=p,
                      expected_return=0.01, position=Position(f"S{i}"))
...
def test_batch_returns_one_decision_per_input():
    engine = DecisionEngine(TH)
    inputs = [DecisionInput(f"S{i}", TODAY, calibrated_probability=0.6,
                            expected_return=0.01) for i in range(5)]
...
def test_out_of_block_performance_is_reported_separately():
    ...
    _, stability = optimise_thresholds_out_of_fold(
        signals_frame(skill=0.3), COSTS
    )
...
def test_cost_drag_above_one_is_flagged():
    ...
    decisions = pd.DataFrame({
        "symbol": ["A"], "decision_date": [TODAY], "signal": ["BUY"],
    })
    outcomes = pd.DataFrame({
        "symbol": ["A"], "date": [TODAY], "y_true": [0.0005],
    })
    text = render_evaluation(
        evaluate_decisions(decisions, outcomes, round_trip_cost=0.0026)
    )
